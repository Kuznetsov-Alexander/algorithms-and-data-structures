    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (!exit)
            {
                ...
                Console.WriteLine("2. Проверить интерфейсы и делегаты (пример)");
            ...
            using (StreamWriter sw = new StreamWriter(filePath, false))
            {
                foreach (var inst in institutes)
                {
                    foreach (var course in inst.Courses)
                    {
                        ...
                        {
                            string line =
                                $"Институт: {inst.Name}, Курс: {course.Number}, Ср. балл: {course.AverageMark:F2}";
        ...
        {
            var student = new Student { LastName = "Петров", Marks = new List<int> { 5, 4, 5 } };

            StudentActions actions = null;
            actions += student.ShowInfo;
            actions += student.EditData;
            actions += student.ShowInfo;
